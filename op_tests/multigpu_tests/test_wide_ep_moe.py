...
For Exp:
  ...
  GLOO_SOCKET_IFNAME=your_if_name(enp193s0f1np1) \
  MORI_SOCKET_IFNAME=your_if_name(enp193s0f1np1) \
  MORI_DEVICE_NIC=ionic \
  MORI_RDMA_DEVICES= 'your_rdma_device' \
  MORI_IB_GID_INDEX=1 \
...
from aiter.fused_moe import (
    fused_topk, get_2stage_cfgs, get_padded_M, situv2,
...
def _test_wide_ep_forward(
    ...
    return op._forward_prequant_impl(
        x_quant, x_scale, topk_weight, topk_ids
    ).clone()


@_test_wide_ep_forward.register_fake
def _test_wide_ep_forward_fake(
    x_quant, x_scale, topk_weight, topk_ids, instance_id, model_dim,
):
    return x_quant.new_empty(
        (topk_ids.shape[0], model_dim), dtype=torch.bfloat16
    )
...
class TestWideEpMoe:
    ...
    def __init__(
        ...
        w1, w1_scale, w2, w2_scale, max_tok_per_rank, gpu_per_node=8,
        swiglu_limit=0.0, activation=None, gate_mode=None,
    ):
    ...
    def _validate_context(self, context):
        ...
        if context.owner_id != self.owner_id or context.generation != self.generation:
            raise RuntimeError("dispatch context is stale or belongs to another instance")
    ...
    def dispatch_prequant(self, x_quant, x_scale, weights, topk_ids):
        ...
        for name, tensor in (
            ("x_quant", x_quant), ("x_scale", x_scale),
            ("weights", weights), ("topk_ids", topk_ids),
        ):
        ...
        context = TestWideEpMoeContext(
            ...
            num_tokens=recv[4], source_topk_ids=topk_ids, source_tokens=tokens,
    ...
    def fused_moe(self, context):
        ...
        return fused_moe(
            context.tokens, self.w1, self.w2, context.weights, context.expert_ids,
            expert_mask=self.expert_mask, activation=self.activation,
            gate_mode=self.gate_mode.value, quant_type=QuantType.per_1x32,
            swiglu_limit=self.swiglu_limit, w1_scale=self.w1_scale,
            w2_scale=self.w2_scale, a1_scale=context.scales,
    ...
    def forward_prequant(self, x_quant, x_scale, weights, topk_ids):
        if torch.compiler.is_compiling():
            return _test_wide_ep_forward(
                x_quant, x_scale, weights, topk_ids,
                self.owner_id, self.model_dim,
            )
...
def _torch_moe_reference(
    x, w1, w2, weights, global_ids, expert_mask, activation, *, swiglu_limit=0.0,
):
...
def _a8w4_stage1_reference_dtype(
    x_quant, w1, w2, weights, activation, gate_mode, requested="auto",
):
    ...
    metadata = get_2stage_cfgs(
        get_padded_M(x_quant.shape[0]), x_quant.shape[1], w2.shape[2] * 2,
        w1.shape[0], weights.shape[1], dtypes.bf16, dtypes.fp8, dtypes.fp4x2,
        QuantType.per_1x32, True, activation, False, 0, 0,
        getattr(w1, "is_shuffled", False) or getattr(w2, "is_shuffled", False),
        gate_mode, is_ep=True, has_stage2_bias=False,
    ...
    if requested == "auto":
        if metadata.run_1stage:
            raise ValueError("automatic A8W4 reference currently requires a two-stage kernel")
        if metadata.fuse_quant not in ("", "fp8"):
            raise ValueError(f"unsupported A8W4 intermediate quantization: {metadata.fuse_quant}")
...
def _torch_a8w4_moe_reference(
    x_quant, x_scale, w1_quant, w1_scale, w2_quant, w2_scale,
    weights, global_ids, expert_mask, activation, *, stage1_dtype=dtypes.bf16,
    swiglu_limit=0.0,
):
    """AITER's A8W4 reference, including its --ref-dtype intermediate choice."""
    if x_quant.shape[0] == 0:
        return torch.empty((0, w2_quant.shape[1]), dtype=dtypes.bf16, device=x_quant.device)
    ...
    out1_ref = torch_moe_stage1(
        x_quant, w1_quant, w2_quant, weights, local_ids,
        dtype=stage1_dtype, activation=activation, quant_type=QuantType.per_1x32,
        a1_scale=x_scale, w1_scale=w1_scale, doweight=False,
    ...
    return torch_moe_stage2(
        a2_quant, w1_quant, w2_quant, weights, local_ids,
        dtype=dtypes.bf16, quant_type=QuantType.per_1x32,
        w2_scale=w2_scale, a2_scale=a2_scale, doweight=True,
    )
...
def _check_a8w4_accuracy(
    reference, actual, *, bs, label, rank, strict_elementwise=False,
):
    ...
    mismatch = checkAllclose(
        reference, actual, rtol=1e-2, atol=1e-2, tol_err_ratio=0.05,
        printLog=False,
    )
    ...
    metrics = {
        key: _reduce_float(value, dist.ReduceOp.MAX)
...
def _torch_a8w4_source_reference(
    source_inputs, w1_quant, w1_scale, w2_quant, w2_scale, expert_mask,
    activation, *, stage1_dtype, chunk_rows=512, swiglu_limit=0.0,
):
    ...
    for rows in local_rows.split(chunk_rows):
        ...
        ref = _torch_a8w4_moe_reference(
            q, scales, w1_quant, w1_scale, w2_quant, w2_scale,
            weights, ids, expert_mask, activation, stage1_dtype=stage1_dtype,
            swiglu_limit=swiglu_limit,
        )
...
def _run_one_bs(
    ...
    w1_a, w1_s, w2_a, w2_s = state.w1, state.w1_scale, state.w2, state.w2_scale
    ...
    if out is not None and bs <= accuracy_max_bs:
        ...
        if quant == "a8w4":
            _check_a8w4_accuracy(
                out, diagnostic_out, bs=bs, label="public_vs_staged_consistency",
                rank=rank, strict_elementwise=strict_elementwise,
            )
        else:
    ...
    kernel_us = (
        {} if staged_only else
        {name: _time_captured_kernel(call) * 1000 for name, call in kernel_calls}
    )
    if debug_wide_ep:
    ...
    if bs <= accuracy_max_bs:
        ...
        # instead of materializing the complete dequantized weights. The legacy
        # A4W4 reference and explicit GEMM diagnostics retain the original path.
        if quant != "a8w4" or os.environ.get("AITER_DEBUG_MX_EP_SYNC", "0") == "1":
        ...
        if quant == "a8w4":
            stage1_ref_dtype = _a8w4_stage1_reference_dtype(
                recv_tok_fp4, w1_a, w2_a, recv_wts, activation,
                op.gate_mode.value, ref_dtype,
            )
            if rank == 0:
                print(
                    f"[A8W4-reference] TPR={bs} capacity={recv_tok_fp4.shape[0]} "
                    f"requested={ref_dtype} stage1_dtype={stage1_ref_dtype} "
                    f"swiglu_limit={op.swiglu_limit}", flush=True,
                )
            ref_moe_out = _torch_a8w4_moe_reference(
                recv_tok_fp4[:total_recv], recv_scale[:total_recv],
                w1_qt, w1_scale, w2_qt, w2_scale,
                recv_wts[:total_recv], recv_idx[:total_recv], expert_mask, activation,
        ...
        if quant == "a8w4":
            accuracy = _check_a8w4_accuracy(
                ref_moe_out, moe_out_correctness[:total_recv],
                bs=bs, label=f"local_moe_vs_quantized_reference_{stage1_ref_dtype}",
                rank=rank, strict_elementwise=strict_elementwise,
            )
        ...
        if end_to_end_reference:
            def make_source_reference(inputs):
                return _torch_a8w4_source_reference(
                    inputs, w1_qt, w1_scale, w2_qt, w2_scale, expert_mask,
                    activation, stage1_dtype=stage1_ref_dtype,
                    swiglu_limit=op.swiglu_limit,
                )
            ...
            _check_a8w4_accuracy(
                source_reference, diagnostic_out, bs=bs,
                label="staged_full_pipeline_vs_source", rank=rank,
                strict_elementwise=strict_elementwise,
            )
            if out is not None:
                _check_a8w4_accuracy(
                    source_reference, out, bs=bs,
                    label="public_full_pipeline_vs_source", rank=rank,
                    strict_elementwise=strict_elementwise,
                )
        if debug_wide_ep:
    ...
    if torch_compile_cudagraph:
        ...
        if quant == "a8w4":
            consistency = _check_a8w4_accuracy(
                out, compiled_out, bs=bs, label="first_compiled_vs_eager_consistency",
                rank=rank, strict_elementwise=strict_elementwise,
            )
        ...
        if source_reference is not None:
            _check_a8w4_accuracy(
                source_reference, compiled_out, bs=bs,
                label="first_compiled_full_pipeline_vs_source", rank=rank,
                strict_elementwise=strict_elementwise,
            )
...
def run_ep16(
    ...
    try:
        ...
        x, topk_weights, topk_ids = _make_local_inputs(
            max_bs, model_dim, experts, topk, rank, run.seed, device,
        )
